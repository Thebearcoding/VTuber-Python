...
class FacialFeatures:
    ...
    def eye_aspect_ratio(image_points, side):
        ...
        eye = eye * (np.linalg.norm(tip_of_eyebrow - image_points[2]) / np.linalg.norm(
            image_points[6] - image_points[2]))
        return eye
    ...
    def detect_iris(image_points, iris_image_points, side):
        ...
        x_rate = (np.dot(vec_p1_iris, vec_p1_p4) / (np.linalg.norm(p1 - p4) + 1e-06)) / (
                    np.linalg.norm(p1 - p4) + 1e-06)
        ...
        y_rate = (np.dot(vec_eye_h_eye_l, vec_eye_h_iris) / (np.linalg.norm(eye_y_high - eye_y_low) + 1e-06)) / (
                    np.linalg.norm(eye_y_high - eye_y_low) + 1e-06)
